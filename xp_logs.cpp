...
#if defined __linux__ || defined __APPLE__
    if (ileaveLength + 1 == ipathLength) continue;
#elif defined _WIN32
    if (ileaveLength + 2 == ipathLength) continue;
#endif
...
char *xp_logs::get_first_file(char *path) {
  ...
  if (n < 0)
    ...
    int index = 0;
    char *name = NULL;
    if (n > 2) name = name_list[2]->d_name;
    while (index < n) free(name_list[index++]);
    free(name_list);
    return name;
  }
  free(name_list);
...
char *xp_logs::get_last_file(char *path) {
  ...
  if (n < 0)
    ...
    int index = 0;
    char *name = NULL;
    if (n > 2) name = name_list[n - 1]->d_name;
    while (index < n) free(name_list[index++]);
    free(name_list);
    return name;
  }
...
int xp_logs::get_dir_size(char *path) {
  ...
  if (access(dirpath, 0) < 0) return -1;
    ...
    if (size >= maxtotalsize)  //超过限制大小，删除最久远日志，添加新日志
    {
      ...
      if (Daily == 1) {
        if (sscanf(name, "%4d-%02d-%02d %02d-%02d-%02d.log", &date[0], &date[1],
                   &date[2], &date[3], &date[4], &date[5]) > 5) {
          //判断是否同一天 t->tm_year + 1900, t->tm_mon + 1, t->tm_mday
          struct tm *t;
          time_t now;
          time(&now);
          t = localtime(&now);
          if ((t->tm_year + 1900) == date[0] && (t->tm_mon + 1) == date[1] &&
              t->tm_mday == date[2])  //同一天
            ;
          else {
            if (creat_log() >= 0) name = get_last_file(logpath);
          }
        }
      }
      if (name) {
